...
$script:NetworkName = "organiza_club_net"
$script:RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path

$script:ComposeTargets = @(
  @{
    Name = "Supabase"
    Project = "organiza-club-supabase"
  ...
  @{
    Name = "Evolution API"
    Project = "organiza-club-evolution"
  ...
  @{
    ...
    Project = "organiza-club-api"
    Directory = Join-Path $script:RepoRoot "apps\api"
    File = "docker-compose.yml"
    Build = $true
  },
  @{
    Name = "Organiza Club Web"
    Project = "organiza-club-web"
...
function Ensure-LocalEnvironment {
  ...
  Ensure-EnvFile -Path $apiEnv -Lines @(
    ...
    "API_PORT=8080",
    "ConnectionStrings__OrganizaClubDb=Host=organiza-club-supabase-db;Port=5432;Database=postgres;Username=supabase_admin;Password=$postgresPassword",
    "Database__ApplyMigrationsOnStartup=true",
    "Jwt__Issuer=organiza-club",
    "Jwt__Audience=organiza-club",
...
function Select-Action {
  Write-Host "Organiza Club local"
...
function Confirm-Destroy {
  ...
  Write-Host ""
  Write-Host "Esta acao remove containers e volumes locais do Organiza Club."
...
function Start-OrganizaClub {
  ...
  Write-Host ""
  Write-Host "Organiza Club local iniciado."
...
function Stop-OrganizaClub {
  ...
  Write-Host ""
  Write-Host "Organiza Club local parado. Volumes preservados."
}

function Destroy-OrganizaClub {
  Confirm-Destroy
  ...
  Write-Host ""
  Write-Host "Organiza Club local removido com volumes."
}
...
switch ($Action) {
  "start" { Start-OrganizaClub }
  "stop" { Stop-OrganizaClub }
  "destroy" { Destroy-OrganizaClub }
}
