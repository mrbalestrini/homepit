...
O fluxo atual permite cadastro com espaço inicial opcional, edicao de perfil somente em modal e
...
- Remover a criacao de espaço inicial do cadastro.
...
- Transferencia explicita de propriedade de espaço.
...
- `apps/api/src/OrganizaClub.Domain/Spaces/*`
- `apps/api/src/OrganizaClub.Application/Auth/*`
- `apps/api/src/OrganizaClub.Application/Spaces/*`
...
- Cadastro sempre cria conta sem espaço inicial.
- Conta sem espaços proprios pode ser excluida imediatamente.
- Conta com ao menos um espaço em que o usuario e `Owner` entra em `PendingSelfDeletion` por 30 dias.
- Ao vencer o prazo, a conta, seus espaços proprios e todos os vinculos desses espaços sao apagados.
...
2. Extrair purge compartilhado de espaços e usuarios com limpeza de object storage.
...
- Unitarios de `AuthService`, `SpaceService` e purge worker.
...
- Cadastro deixa de pedir espaço inicial.
- Usuario pode cancelar a propria conta conforme o cenario com ou sem espaços proprios.
...
proprio com espaços, pagina dedicada de perfil e painel global de usuarios do superadmin.
