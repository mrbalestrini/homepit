...
2. `Core` e `Project` existentes serao reaproveitados como classificacao opcional no
   financeiro.
3. Cada space tera no maximo um `FinancePeriod` por ano/mes.
...
11. Categorias financeiras sao vinculadas por space, mas ficam visiveis apenas dentro
    do modulo `/finance`.
...
21. A revisão da importação poderá criar categorias novas por nome dentro do próprio fluxo,
    mas núcleos e projetos continuarão limitados aos registros já existentes.
