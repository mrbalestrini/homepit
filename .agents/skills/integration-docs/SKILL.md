...
description: Criar, revisar e manter a documentação de consumo das integrações REST e MCP do Organiza Club. Use quando alterar o OpenAPI externo, tools MCP, OAuth, bridge stdio, exemplos de automação, receitas para IA ou a documentação exibida na aba Conexão.
---
...
2. Trate `contracts/openapi/organiza-club.integrations.v1.yaml` como a fonte de nomes, campos, operações e erros REST; trate a implementação MCP como fonte de disponibilidade de tools/resources.
...
5. Mostre variáveis como `ORGANIZA_INTEGRATION_TOKEN` ou `SEU_TOKEN`; nunca inclua tokens, hashes, pepper, dados financeiros reais, URL interna ou segredo nos exemplos.
...
- Preserve a fronteira: API externa não recebe `X-Space-Id`; o Espaço é definida pela conexão.
