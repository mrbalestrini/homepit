...
public sealed class InstitutionalPageServiceTests
{
    ...
    public async Task Public_content_uses_defaults_without_persisting_a_page()
    {
        ...
        Assert.Equal("Organiza Club", result.BrandName);
    ...
    public async Task Rejects_invalid_primary_cta_url_and_list_size()
    {
        ...
        Assert.Contains("caminho interno ou link HTTP/HTTPS", urlException.Message);
        Assert.Contains("entre 1 e 6 benefícios", listException.Message);
    }

    [Fact]
    public async Task Accepts_internal_primary_cta_path()
    {
        var context = CreateContext(SystemRole.SuperAdmin);

        var result = await context.Service.UpdateAsync(
            CreateRequest() with { PrimaryCtaUrl = "/projects" },
            CancellationToken.None);

        Assert.Equal("/projects", result.PrimaryCtaUrl);
    }
