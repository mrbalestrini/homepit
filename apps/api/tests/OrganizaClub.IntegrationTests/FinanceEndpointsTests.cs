...
{
    [Fact]
    public async Task Finance_entry_and_recurring_template_crud_works_with_space_header()
    {
        ...
        Assert.Equal(seed.CoreId, createdTemplate.CoreId);
    ...
    public async Task Credit_card_transaction_import_creates_categories_and_purchases_atomically()
    {
        ...
        var importError = importResponse.IsSuccessStatusCode ? null : await importResponse.Content.ReadAsStringAsync();
        Assert.True(importResponse.IsSuccessStatusCode, importError);
        ...
        var verificationDb = verificationScope.ServiceProvider.GetRequiredService<OrganizaClubDbContext>();
        Assert.Equal(2, await verificationDb.CreditCardTransactions.CountAsync(item => item.CreditCardAccountId == createdCard.Id));
        Assert.Single(await verificationDb.FinanceCategories.Where(item => item.SpaceId == seed.SpaceId && item.Name == "Categoria importada").ToArrayAsync());
        ...
        var rollbackDb = rollbackScope.ServiceProvider.GetRequiredService<OrganizaClubDbContext>();
        Assert.Equal(2, await rollbackDb.CreditCardTransactions.CountAsync(item => item.CreditCardAccountId == createdCard.Id));
        Assert.Empty(await rollbackDb.FinanceCategories.Where(item => item.SpaceId == seed.SpaceId && item.Name == "Nova categoria").ToArrayAsync());
    }
    ...
    public async Task Finance_period_generation_keeps_a_single_period_per_month()
    {
        ...
        var julyPeriods = await verificationDb.FinancePeriods
            .CountAsync(period => period.SpaceId == seed.SpaceId && period.Year == 2026 && period.Month == 7);

        Assert.Equal(1, julyPeriods);
    }

    [Fact]
    public async Task Deleting_project_and_core_nulls_finance_links()
    {
        await using var factory = new OrganizaClubApiFactory();
        using var client = factory.CreateClient();
        var seed = await SeedAsync(factory);
        ...
        {
            var db = afterProjectScope.ServiceProvider.GetRequiredService<OrganizaClubDbContext>();
            Assert.All(await db.FinanceEntries.Where(item => item.SpaceId == seed.SpaceId).ToArrayAsync(), item => Assert.Null(item.ProjectId));
            Assert.All(await db.FinanceRecurringTemplates.Where(item => item.SpaceId == seed.SpaceId).ToArrayAsync(), item => Assert.Null(item.ProjectId));
            Assert.All(await db.CreditCardTransactions.Where(item => item.SpaceId == seed.SpaceId).ToArrayAsync(), item => Assert.Null(item.ProjectId));
            Assert.All(await db.FinanceEntries.Where(item => item.SpaceId == seed.SpaceId).ToArrayAsync(), item => Assert.Equal(seed.CoreId, item.CoreId));
        }
        ...
        var deleteCoreResponse = await SendAuthorizedAsync(
            ...
            seed.SpaceId,
            HttpMethod.Delete,
            $"/api/cores/{seed.CoreId}");

        Assert.Equal(HttpStatusCode.NoContent, deleteCoreResponse.StatusCode);

        await using var afterCoreScope = factory.Services.CreateAsyncScope();
        var verificationDb = afterCoreScope.ServiceProvider.GetRequiredService<OrganizaClubDbContext>();
        Assert.All(await verificationDb.FinanceEntries.Where(item => item.SpaceId == seed.SpaceId).ToArrayAsync(), item => Assert.Null(item.CoreId));
        Assert.All(await verificationDb.FinanceRecurringTemplates.Where(item => item.SpaceId == seed.SpaceId).ToArrayAsync(), item => Assert.Null(item.CoreId));
        Assert.All(await verificationDb.CreditCardTransactions.Where(item => item.SpaceId == seed.SpaceId).ToArrayAsync(), item => Assert.Null(item.CoreId));
    }

    [Fact]
    public async Task New_space_starts_with_default_finance_categories()
    {
        await using var factory = new OrganizaClubApiFactory();
        using var client = factory.CreateClient();
        var seed = await SeedAsync(factory);

        var createSpaceResponse = await client.SendAsync(new HttpRequestMessage(HttpMethod.Post, "/api/spaces")
        {
            Headers =
            {
                Authorization = new AuthenticationHeaderValue("Bearer", seed.OwnerAccessToken)
            },
            Content = JsonContent.Create(new { name = "Espaço Novo" })
        });

        createSpaceResponse.EnsureSuccessStatusCode();
        var createdSpace = await createSpaceResponse.Content.ReadFromJsonAsync<SpaceResponse>(JsonSerializerOptions.Web);
    ...
    private static async Task<SeedResult> SeedAsync(OrganizaClubApiFactory factory)
    {
        ...
        db.Spaces.Add(space);
        db.SpaceMembers.AddRange(ownerMember, member);
        await db.SaveChangesAsync();

        db.FinanceCategories.AddRange(FinanceCategoryCatalog.CreateDefaults(space.Id, ownerMember.Id));
        await db.SaveChangesAsync();
        ...
        db.UserSubscriptions.Add(new UserSubscription
        {
            ...
            AdminNote = "Seed de teste para permitir o segundo espaço."
        });
        await db.SaveChangesAsync();
        ...
        return new SeedResult(
            ...
            space.Id,
            ownerMember.Id,
            db.FinanceCategories.First(item => item.SpaceId == space.Id && item.Name == "Salário").Id,
            core.Id,
            project.Id);
    }
    ...
    private sealed record SpaceResponse(Guid Id, string Name, string Role, DateTimeOffset CreatedAt);

    private sealed record FinanceRecurringTemplateResponse(Guid Id, Guid? CategoryId, Guid? CoreId, Guid? ProjectId);
    ...
    private sealed class OrganizaClubApiFactory : WebApplicationFactory<Program>, IAsyncDisposable
    {
        ...
        protected override void ConfigureWebHost(Microsoft.AspNetCore.Hosting.IWebHostBuilder builder)
        {
            ...
            builder.ConfigureServices(services =>
            {
                ...
                services.AddDbContext<OrganizaClubDbContext>(options => options.UseSqlite(connection));
        ...
        private static void SeedMigrationHistory(OrganizaClubDbContext db)
        {
