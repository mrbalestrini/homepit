...
public sealed class ToolImprovementSuggestionEndpointsTests
{
    ...
    public async Task Authenticated_user_can_submit_and_superadmin_can_triage_suggestions()
    {
        ...
        var userAuth = await RegisterAndAuthenticateAsync(client, "user@organiza.club", "User");
        ...
        Assert.Equal("user@organiza.club", suggestion.UserEmail);
    ...
    public async Task Non_superadmin_cannot_access_admin_suggestions()
    {
        ...
        var userAuth = await RegisterAndAuthenticateAsync(client, "member@organiza.club", "Member");
    ...
    private sealed record AuthResponse(string AccessToken, string RefreshToken, string ExpiresAt, AuthUserResponse User, IReadOnlyCollection<object> Spaces);
