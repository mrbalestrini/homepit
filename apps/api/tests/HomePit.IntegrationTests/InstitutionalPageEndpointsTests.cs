...
public sealed class InstitutionalPageEndpointsTests
{
    ...
    public async Task Public_content_is_available_without_authentication()
    {
        ...
        Assert.Equal("HomePit", content.BrandName);
    ...
    public async Task Superadmin_updates_content_and_it_becomes_public_immediately()
    {
        ...
        var updateResponse = await SendAuthorizedAsync(
            ...
            JsonContent.Create(CreateRequest("Título publicado")));
        ...
        Assert.Equal("Título publicado", publicResponse.HeroTitle);
    ...
    private static async Task<string> SeedAccessTokenAsync(HomePitApiFactory factory, SystemRole role)
    {
        ...
        {
            Email = $"{role.ToString().ToLowerInvariant()}-{Guid.NewGuid():N}@homepit.dev",
            PasswordHash = "hash",
