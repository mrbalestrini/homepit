...
internal static class TestImageFactory
{
    ...
    public static byte[] CreateJpeg(int width, int height, ushort? orientation = null)
    {
    ...
    public static byte[] CreateAnimatedGif()
    {
        using var image = new Image<Rgba32>(48, 48, new Rgba32(255, 0, 0, 255));
        image.Frames.RootFrame.Metadata.GetGifMetadata().FrameDelay = 10;

        using var secondImage = new Image<Rgba32>(48, 48, new Rgba32(0, 0, 255, 255));
        secondImage.Frames.RootFrame.Metadata.GetGifMetadata().FrameDelay = 10;
        image.Frames.AddFrame(secondImage.Frames.RootFrame);

        return Save(image, new GifEncoder());
    }
