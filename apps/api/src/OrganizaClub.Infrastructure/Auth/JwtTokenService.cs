...
public sealed class JwtTokenService(IOptions<JwtOptions> options, TimeProvider timeProvider) : ITokenService
{
    ...
    public string CreateAccessToken(AppUser user, IReadOnlyCollection<SpaceMember> memberships)
    {
        ...
        claims.AddRange(memberships.Select(member => new Claim("space", member.SpaceId.ToString())));
        claims.AddRange(memberships.Select(member => new Claim($"space:{member.SpaceId}:role", member.Role.ToString())));
