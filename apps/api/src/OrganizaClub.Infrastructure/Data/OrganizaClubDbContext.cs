...
public sealed class OrganizaClubDbContext(DbContextOptions<OrganizaClubDbContext> options)
    : DbContext(options), IOrganizaClubDbContext
{
    ...
    {
        modelBuilder.HasDefaultSchema("organiza_club");
    ...
    private static void ConfigureSpaces(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Space>(builder =>
        {
            ...
            builder.Property(space => space.Name).HasMaxLength(160).IsRequired();
            builder.HasIndex(space => space.CreatedByUserId);
            builder.HasOne(space => space.CreatedByUser)
                .WithMany(user => user.CreatedSpaces)
                .HasForeignKey(space => space.CreatedByUserId)
                .OnDelete(DeleteBehavior.Restrict);
        });

        modelBuilder.Entity<SpaceMember>(builder =>
        {
            builder.ToTable("space_members");
            builder.Property(member => member.Role).HasConversion<string>().HasMaxLength(40).IsRequired();
            builder.HasIndex(member => new { member.SpaceId, member.UserId }).IsUnique();
            ...
            builder.HasOne(member => member.User)
                .WithMany(user => user.SpaceMembers)
                .HasForeignKey(member => member.UserId)
                .OnDelete(DeleteBehavior.Cascade);
        });

        modelBuilder.Entity<SpaceInvitation>(builder =>
        {
            builder.ToTable("space_invitations");
            ...
            builder.HasIndex(item => new { item.SpaceId, item.InviteeEmail }).IsUnique();
            ...
            builder.HasOne(item => item.InviterUser)
                .WithMany(user => user.SentSpaceInvitations)
    ...
    private static void ConfigureIntegrationConcurrency(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Core>().Property(item => item.UpdatedAt).IsConcurrencyToken();
    ...
    private static void ConfigureIntegrations(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<IntegrationConnection>(builder =>
        {
            ...
            builder.HasIndex(item => new { item.UserId, item.SpaceId, item.ExpiresAt });
    ...
    private static void ConfigureGsm(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<GsmNumber>(builder =>
        {
            ...
            builder.Property(item => item.Status).HasConversion<string>().HasMaxLength(40).IsRequired();
            builder.HasIndex(item => new { item.SpaceId, item.NormalizedNumber }).IsUnique();
            builder.HasOne(item => item.Space)
                .WithMany(space => space.GsmNumbers)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
        ...
        modelBuilder.Entity<GsmRecharge>(builder =>
        {
            ...
            builder.HasIndex(item => new { item.SpaceId, item.GsmNumberId, item.RechargedOn });
            builder.HasOne(item => item.Space)
                .WithMany(space => space.GsmRecharges)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
    ...
    private static void ConfigureFinance(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<FinanceCategory>(builder =>
        {
            ...
            builder.HasIndex(item => new { item.SpaceId, item.Name }).IsUnique();
            builder.HasIndex(item => new { item.SpaceId, item.SortOrder });
            builder.HasOne(item => item.Space)
                .WithMany(space => space.FinanceCategories)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(item => item.CreatedByMember)
        ...
        modelBuilder.Entity<FinancePeriod>(builder =>
        {
            ...
            builder.HasIndex(period => new { period.SpaceId, period.Year, period.Month }).IsUnique();
            builder.HasOne(period => period.Space)
                .WithMany(space => space.FinancePeriods)
                .HasForeignKey(period => period.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
        });
        ...
        modelBuilder.Entity<FinanceRecurringTemplate>(builder =>
        {
            ...
            builder.HasIndex(item => new { item.SpaceId, item.IsActive, item.Recurrence });
            builder.HasOne(item => item.Space)
                .WithMany(space => space.FinanceRecurringTemplates)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(item => item.CreatedByMember)
                ...
                .HasForeignKey(item => item.CreatedByMemberId)
                .OnDelete(DeleteBehavior.SetNull);
            builder.HasOne(item => item.Core)
                .WithMany()
                .HasForeignKey(item => item.CoreId)
                .OnDelete(DeleteBehavior.SetNull);
        ...
        modelBuilder.Entity<FinanceEntry>(builder =>
        {
            ...
            builder.HasIndex(item => new { item.SpaceId, item.FinancePeriodId, item.ReferenceDate });
            builder.HasIndex(item => item.CreditCardStatementId).IsUnique();
            builder.HasOne(item => item.Space)
                .WithMany(space => space.FinanceEntries)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
        ...
        modelBuilder.Entity<Asset>(builder =>
        {
            ...
            builder.Property(item => item.Notes).HasMaxLength(4000);
            builder.HasIndex(item => new { item.SpaceId, item.Type, item.Title });
            builder.HasOne(item => item.Space)
                .WithMany(space => space.Assets)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(item => item.CreatedByMember)
        ...
        modelBuilder.Entity<CreditCardAccount>(builder =>
        {
            ...
            builder.HasIndex(item => new { item.SpaceId, item.Name }).IsUnique();
            builder.HasOne(item => item.Space)
                .WithMany(space => space.CreditCardAccounts)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(item => item.CreatedByMember)
        ...
        modelBuilder.Entity<CreditCardStatement>(builder =>
        {
            ...
            builder.HasOne(item => item.Space)
                .WithMany(space => space.CreditCardStatements)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
        ...
        modelBuilder.Entity<CreditCardTransaction>(builder =>
        {
            ...
            builder.HasOne(item => item.Space)
                .WithMany(space => space.CreditCardTransactions)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(item => item.CreditCardAccount)
    ...
    {
        modelBuilder.Entity<Core>(builder =>
        {
            builder.ToTable("cores");
            builder.Property(core => core.Name).HasMaxLength(160).IsRequired();
            builder.Property(core => core.ImageUrl).HasMaxLength(2000);
            builder.Property(core => core.ImageObjectKey).HasMaxLength(512);
            builder.Property(core => core.ImageContentType).HasMaxLength(120);
            builder.HasIndex(core => new { core.SpaceId, core.Name }).IsUnique();
            builder.HasOne(core => core.Space)
                .WithMany(space => space.Cores)
                .HasForeignKey(core => core.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(core => core.CreatedByMember)
                .WithMany(member => member.CreatedCores)
                .HasForeignKey(core => core.CreatedByMemberId)
                .OnDelete(DeleteBehavior.SetNull);
        });
        ...
        modelBuilder.Entity<Project>(builder =>
        {
            ...
            builder.HasIndex(project => new { project.SpaceId, project.CoreId, project.Name }).IsUnique();
            builder.HasOne(project => project.Core)
                .WithMany(core => core.Projects)
                .HasForeignKey(project => project.CoreId)
                .OnDelete(DeleteBehavior.Cascade);
        ...
        modelBuilder.Entity<MemberEffortAllocation>(builder =>
        {
            builder.ToTable("member_effort_allocations", tableBuilder =>
            {
                ...
                tableBuilder.HasCheckConstraint(
                    ...
                    "(\"ScopeType\" = 'Space' AND \"CoreId\" IS NULL AND \"ProjectId\" IS NULL) OR (\"ScopeType\" = 'Core' AND \"CoreId\" IS NOT NULL AND \"ProjectId\" IS NULL) OR (\"ScopeType\" = 'Project' AND \"CoreId\" IS NULL AND \"ProjectId\" IS NOT NULL)");
            });
            ...
            builder.HasIndex(item => new { item.SpaceMemberId, item.Weekday })
                .IsUnique()
                .HasFilter("\"ScopeType\" = 'Space'");
            builder.HasIndex(item => new { item.SpaceMemberId, item.CoreId, item.Weekday })
                .IsUnique()
                .HasFilter("\"ScopeType\" = 'Core'");
            builder.HasIndex(item => new { item.SpaceMemberId, item.ProjectId, item.Weekday })
                .IsUnique()
                .HasFilter("\"ScopeType\" = 'Project'");
            builder.HasOne(item => item.Space)
                .WithMany(space => space.MemberEffortAllocations)
                .HasForeignKey(item => item.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(item => item.SpaceMember)
                .WithMany(member => member.EffortAllocations)
                .HasForeignKey(item => item.SpaceMemberId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(item => item.Core)
                .WithMany(core => core.EffortAllocations)
                .HasForeignKey(item => item.CoreId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(item => item.Project)
        ...
        modelBuilder.Entity<ActivityComment>(builder =>
        {
            ...
            builder.HasIndex(comment => new { comment.SpaceId, comment.ActivityId, comment.CreatedAt });
    ...
    private static void ConfigureNotifications(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<NotificationPreference>(builder =>
        {
            ...
            builder.HasIndex(preference => preference.SpaceMemberId).IsUnique();
            builder.HasOne(preference => preference.Space)
                .WithMany()
                .HasForeignKey(preference => preference.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(preference => preference.SpaceMember)
                .WithOne(member => member.NotificationPreference)
                .HasForeignKey<NotificationPreference>(preference => preference.SpaceMemberId)
                .OnDelete(DeleteBehavior.Cascade);
        });
        ...
        modelBuilder.Entity<NotificationRun>(builder =>
        {
            ...
            builder.HasIndex(run => new { run.SpaceId, run.SpaceMemberId, run.Kind, run.LocalDate }).IsUnique();
            builder.HasOne(run => run.Space)
                .WithMany(space => space.NotificationRuns)
                .HasForeignKey(run => run.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
        });
    }
    ...
    private static void ConfigurePrompts(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<Prompt>(builder =>
        {
            ...
            builder.HasIndex(prompt => new { prompt.SpaceId, prompt.IsArchived, prompt.UpdatedAt });
            builder.HasIndex(prompt => new { prompt.SpaceId, prompt.IsArchived, prompt.CoreId, prompt.UpdatedAt });
            builder.HasOne(prompt => prompt.Space)
                .WithMany(space => space.Prompts)
                .HasForeignKey(prompt => prompt.SpaceId)
                .OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(prompt => prompt.CreatedByMember)
                ...
                .OnDelete(DeleteBehavior.SetNull);
            builder.HasOne(prompt => prompt.Core)
                .WithMany(core => core.Prompts)
                .HasForeignKey(prompt => prompt.CoreId)
                .OnDelete(DeleteBehavior.SetNull);
        });
        ...
        modelBuilder.Entity<PromptCategory>(builder =>
        {
            ...
            builder.HasIndex(category => new { category.SpaceId, category.Name }).IsUnique();
            builder.HasOne(category => category.Space)
                .WithMany(space => space.PromptCategories)
