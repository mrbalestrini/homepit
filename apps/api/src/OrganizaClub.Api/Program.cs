...
using OrganizaClub.Api.Mcp;
using OrganizaClub.Application;
...
using OrganizaClub.Infrastructure;
...
builder.Services.AddOrganizaClubApplication();
builder.Services.AddOrganizaClubInfrastructure(builder.Configuration);
...
if (oauthEnabled)
{
    ...
    builder.Services.AddOpenIddict()
        .AddCore(options => options.UseEntityFrameworkCore().UseDbContext<OrganizaClubDbContext>())
        .AddServer(options =>
        {
...
app.Use(async (context, next) =>
{
    ...
    {
        context.Response.Headers.WWWAuthenticate = $"Bearer resource_metadata=\"{oauthOptions.Issuer.TrimEnd('/')}/.well-known/oauth-protected-resource/mcp\", scope=\"organiza.read organiza.write\"";
    }
});
...
{
    await app.Services.MigrateOrganizaClubDatabaseAsync();
}
else
{
    await app.Services.EnsureNoPendingOrganizaClubMigrationsAsync();
}
await app.Services.EnsureOrganizaClubObjectStorageAsync();

app.MapGet("/health", () => Results.Ok(new { status = "ok" }));
app.MapGet("/api/system/info", () => Results.Ok(new
{
    name = "Organiza Club API",
    version = "0.5.0",
...
if (oauthEnabled)
{
    app.MapGet("/.well-known/oauth-protected-resource/mcp", () => Results.Ok(new
    {
        ...
        scopes_supported = new[] { "organiza.read", "organiza.write" },
        ...
        CancellationToken cancellationToken) =>
    {
        ...
        descriptor.Permissions.Add(OpenIddictConstants.Permissions.Prefixes.Scope + "organiza.read");
        descriptor.Permissions.Add(OpenIddictConstants.Permissions.Prefixes.Scope + "organiza.write");
        ...
        CancellationToken cancellationToken) =>
    {
        ...
        identity.SetClaim("integration_space_id", interaction.IntegrationConnection.SpaceId.ToString());
        ...
        await context.RequestServices.GetRequiredService<OrganizaClubDbContext>().SaveChangesAsync(cancellationToken);
...
auth.MapPost("/register", async (RegisterRequest request, AuthService service, CancellationToken cancellationToken) =>
    Results.Created("/api/spaces", await service.RegisterAsync(request, cancellationToken)));
...
api.MapGet("/spaces", async (SpaceService service, CancellationToken cancellationToken) =>
    Results.Ok(await service.ListAsync(cancellationToken)));
api.MapPost("/spaces", async (CreateSpaceRequest request, SpaceService service, CancellationToken cancellationToken) =>
    Results.Created("/api/spaces", await service.CreateAsync(request, cancellationToken)));
api.MapPut("/spaces/{id:guid}", async (
    Guid id,
    UpdateSpaceRequest request,
    SpaceService service,
    CancellationToken cancellationToken) =>
        Results.Ok(await service.UpdateAsync(id, request, cancellationToken)));
api.MapDelete("/spaces/{id:guid}", async (
    Guid id,
    SpaceService service,
    CancellationToken cancellationToken) =>
{
...
api.MapGet("/spaces/members", async (SpaceService service, CancellationToken cancellationToken) =>
    Results.Ok(await service.ListMembersAsync(cancellationToken)));
api.MapPost("/spaces/share", async (ShareSpaceRequest request, SpaceService service, CancellationToken cancellationToken) =>
    Results.Created("/api/spaces/invitations", await service.ShareAsync(request, cancellationToken)));
api.MapGet("/spaces/invitations", async (SpaceService service, CancellationToken cancellationToken) =>
    Results.Ok(await service.ListInvitationsAsync(cancellationToken)));
api.MapPost("/spaces/invitations/{id:guid}/accept", async (
    Guid id,
    SpaceService service,
    CancellationToken cancellationToken) =>
        Results.Ok(await service.AcceptInvitationAsync(id, cancellationToken)));
api.MapPost("/spaces/invitations/{id:guid}/decline", async (
    Guid id,
    SpaceService service,
    CancellationToken cancellationToken) =>
{
...
api.MapGet("/cores", async (ProjectService service, CancellationToken cancellationToken) =>
    Results.Ok(await service.ListCoresAsync(cancellationToken)));
api.MapPost("/cores", async (CreateCoreRequest request, ProjectService service, CancellationToken cancellationToken) =>
    Results.Created("/api/cores", await service.CreateCoreAsync(request, cancellationToken)));
api.MapPut("/cores/{id:guid}", async (
    Guid id,
    UpdateCoreRequest request,
    ProjectService service,
    CancellationToken cancellationToken) =>
        Results.Ok(await service.UpdateCoreAsync(id, request, cancellationToken)));
api.MapDelete("/cores/{id:guid}", async (
    Guid id,
    ProjectService service,
    CancellationToken cancellationToken) =>
{
    await service.DeleteCoreAsync(id, cancellationToken);
    return Results.NoContent();
});
api.MapPost("/cores/{id:guid}/image", async (
    Guid id,
    HttpRequest request,
    ProjectService service,
    CancellationToken cancellationToken) =>
{
    if (!request.HasFormContentType)
    {
        throw new ValidationException("Envie a imagem do núcleo em multipart/form-data.");
    }
    ...
    if (file is null)
    {
        throw new ValidationException("Selecione uma imagem para o núcleo.");
    }

    await using var stream = file.OpenReadStream();
    return Results.Ok(await service.UploadCoreImageAsync(id, stream, file.Length, file.ContentType, cancellationToken));
});
api.MapGet("/cores/{id:guid}/image", async (
    Guid id,
    HttpContext context,
    ProjectService service,
    CancellationToken cancellationToken) =>
{
    var image = await service.GetCoreImageAsync(id, cancellationToken);
    context.Response.Headers.CacheControl = "no-store";
    return Results.File(image.Content, image.ContentType);
});
api.MapDelete("/cores/{id:guid}/image", async (
    Guid id,
    ProjectService service,
    CancellationToken cancellationToken) =>
        Results.Ok(await service.DeleteCoreImageAsync(id, cancellationToken)));

api.MapGet("/projects", async (Guid? coreId, ProjectService service, CancellationToken cancellationToken) =>
    Results.Ok(await service.ListProjectsAsync(coreId, cancellationToken)));
...
api.MapGet("/activities", async (
    Guid? projectId,
    OrganizaClub.Domain.Projects.ActivityStatus? status,
    ProjectService service,
    CancellationToken cancellationToken) =>
...
api.MapGet("/prompts", async (
    ...
    CancellationToken cancellationToken) =>
        Results.Ok(await service.ListPromptsAsync(
            ...
            ReadOptionalGuidQuery(request, "coreId"),
            ReadOptionalBoolQuery(request, "withoutCore") ?? false,
...
// space context, so callers cannot select a space with X-Space-Id.
...
integrationFinance.MapGet("/periods", async (HttpRequest request, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationFinance.MapGet("/categories", async (HttpRequest request, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationFinance.MapPost("/categories", async (
    ...
    IntegrationIdempotencyService idempotency, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationFinance.MapPut("/categories/{id:guid}", async (Guid id, UpdateFinanceCategoryRequest request, HttpRequest httpRequest, HttpResponse response, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationFinance.MapDelete("/categories/{id:guid}", async (Guid id, HttpRequest httpRequest, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.FinanceCategories, cancellationToken);
    await service.DeleteCategoryAsync(id, cancellationToken, expected);
    return Results.NoContent();
});
integrationFinance.MapGet("/recurring-templates", async (HttpRequest request, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationFinance.MapPut("/recurring-templates/{id:guid}", async (Guid id, UpdateFinanceRecurringTemplateRequest request, HttpRequest httpRequest, HttpResponse response, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationFinance.MapDelete("/recurring-templates/{id:guid}", async (Guid id, HttpRequest httpRequest, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.FinanceRecurringTemplates, cancellationToken);
    await service.DeleteRecurringTemplateAsync(id, cancellationToken, expected);
    return Results.NoContent();
});
integrationFinance.MapGet("/entries", async (HttpRequest request, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationFinance.MapPut("/entries/{id:guid}", async (Guid id, UpdateFinanceEntryRequest request, HttpRequest httpRequest, HttpResponse response, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationFinance.MapDelete("/entries/{id:guid}", async (Guid id, HttpRequest httpRequest, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.FinanceEntries, cancellationToken);
    await service.DeleteEntryAsync(id, cancellationToken, expected);
    return Results.NoContent();
});
integrationFinance.MapGet("/assets", async (HttpRequest request, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationFinance.MapPut("/assets/{id:guid}", async (Guid id, UpdateAssetRequest request, HttpRequest httpRequest, HttpResponse response, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationFinance.MapDelete("/assets/{id:guid}", async (Guid id, HttpRequest httpRequest, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.Assets, cancellationToken);
    await service.DeleteAssetAsync(id, cancellationToken, expected);
    return Results.NoContent();
});
integrationFinance.MapGet("/assets/{id:guid}/valuations", async (Guid id, HttpRequest request, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationFinance.MapPut("/assets/{id:guid}/valuations/{valuationId:guid}", async (Guid id, Guid valuationId, UpdateAssetValuationRequest request, HttpRequest httpRequest, HttpResponse response, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationFinance.MapDelete("/assets/{id:guid}/valuations/{valuationId:guid}", async (Guid id, Guid valuationId, HttpRequest httpRequest, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(valuationId, httpRequest.Headers.IfMatch.ToString(), db.AssetValuations, cancellationToken);
    await service.DeleteAssetValuationAsync(id, valuationId, cancellationToken, expected);
    return Results.NoContent();
});
integrationFinance.MapGet("/credit-cards", async (HttpRequest request, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationFinance.MapPut("/credit-cards/{id:guid}", async (Guid id, UpdateCreditCardAccountRequest request, HttpRequest httpRequest, HttpResponse response, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationFinance.MapDelete("/credit-cards/{id:guid}", async (Guid id, HttpRequest httpRequest, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.CreditCardAccounts, cancellationToken);
    await service.DeleteCreditCardAccountAsync(id, cancellationToken, expected);
    return Results.NoContent();
});
integrationFinance.MapGet("/credit-cards/{id:guid}/transactions", async (Guid id, HttpRequest request, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationFinance.MapDelete("/credit-cards/{id:guid}/transactions/{transactionId:guid}", async (Guid id, Guid transactionId, HttpRequest httpRequest, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(transactionId, httpRequest.Headers.IfMatch.ToString(), db.CreditCardTransactions, cancellationToken);
    await service.DeleteCreditCardTransactionAsync(id, transactionId, cancellationToken, expected);
    return Results.NoContent();
});
integrationFinance.MapGet("/credit-cards/{id:guid}/statements", async (Guid id, HttpRequest request, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationFinance.MapPost("/credit-cards/{id:guid}/statements", async (Guid id, CreateCreditCardStatementRequest request, HttpRequest httpRequest, HttpResponse response, FinanceService service, IntegrationIdempotencyService idempotency, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationFinance.MapPut("/credit-cards/{id:guid}/statements/{statementId:guid}", async (Guid id, Guid statementId, UpdateCreditCardStatementRequest request, HttpRequest httpRequest, HttpResponse response, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationFinance.MapDelete("/credit-cards/{id:guid}/statements/{statementId:guid}", async (Guid id, Guid statementId, HttpRequest httpRequest, FinanceService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(statementId, httpRequest.Headers.IfMatch.ToString(), db.CreditCardStatements, cancellationToken);
...
integrationProjects.MapGet("/cores", async (HttpRequest request, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
    Results.Ok(await rest.PageAsync((await service.ListCoresAsync(cancellationToken)).Select(IntegrationExternalDto.ToExternal).ToArray(), item => item.Id, db.Cores,
        "projects.cores", ReadOptionalQueryString(request, "cursor"), ReadOptionalIntQuery(request, "limit"), cancellationToken)));
integrationProjects.MapPost("/cores", async (CreateCoreRequest request, HttpRequest httpRequest, HttpResponse response, ProjectService service, IntegrationIdempotencyService idempotency, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var result = await idempotency.ExecuteAsync("projects_create_core", httpRequest.Headers["Idempotency-Key"].ToString(), request,
        () => service.CreateCoreAsync(request, cancellationToken), cancellationToken);
    var resource = await rest.ResourceAsync(IntegrationExternalDto.ToExternal(result), result.Id, db.Cores, cancellationToken);
    IntegrationRestSupport.SetEtag(response, resource.Etag);
    return Results.Created("/api/integrations/v1/projects/cores", resource);
});
integrationProjects.MapPut("/cores/{id:guid}", async (Guid id, UpdateCoreRequest request, HttpRequest httpRequest, HttpResponse response, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.Cores, cancellationToken);
    var updated = await service.UpdateCoreAsync(id, request, cancellationToken, expected);
    var resource = await rest.ResourceAsync(IntegrationExternalDto.ToExternal(updated), id, db.Cores, cancellationToken);
    IntegrationRestSupport.SetEtag(response, resource.Etag);
    return Results.Ok(resource);
});
integrationProjects.MapDelete("/cores/{id:guid}", async (Guid id, HttpRequest httpRequest, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.Cores, cancellationToken);
    await service.DeleteCoreAsync(id, cancellationToken, expected);
    return Results.NoContent();
});
integrationProjects.MapGet("/items", async (HttpRequest request, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var coreId = ReadOptionalGuidQuery(request, "coreId");
    return Results.Ok(await rest.PageAsync((await service.ListProjectsAsync(coreId, cancellationToken)).Select(IntegrationExternalDto.ToExternal).ToArray(), item => item.Id, db.Projects,
        $"projects.items:{coreId}", ReadOptionalQueryString(request, "cursor"), ReadOptionalIntQuery(request, "limit"), cancellationToken));
});
integrationProjects.MapPost("/items", async (CreateProjectRequest request, HttpRequest httpRequest, HttpResponse response, ProjectService service, IntegrationIdempotencyService idempotency, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationProjects.MapPut("/items/{id:guid}", async (Guid id, UpdateProjectRequest request, HttpRequest httpRequest, HttpResponse response, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationProjects.MapDelete("/items/{id:guid}", async (Guid id, HttpRequest httpRequest, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.Projects, cancellationToken);
    await service.DeleteProjectAsync(id, cancellationToken, expected);
    return Results.NoContent();
});
integrationProjects.MapGet("/activities", async (HttpRequest request, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var projectId = ReadOptionalGuidQuery(request, "projectId");
    var statusRaw = ReadOptionalQueryString(request, "status");
    OrganizaClub.Domain.Projects.ActivityStatus? status = statusRaw is null ? null : Enum.TryParse<OrganizaClub.Domain.Projects.ActivityStatus>(statusRaw, true, out var parsed)
...
integrationProjects.MapPost("/activities", async (CreateActivityRequest request, HttpRequest httpRequest, HttpResponse response, ProjectService service, IntegrationIdempotencyService idempotency, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationProjects.MapPut("/activities/{id:guid}", async (Guid id, UpdateActivityRequest request, HttpRequest httpRequest, HttpResponse response, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
...
integrationProjects.MapPatch("/activities/{id:guid}/status", async (Guid id, UpdateActivityStatusRequest request, HttpRequest httpRequest, HttpResponse response, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.Activities, cancellationToken);
...
integrationProjects.MapDelete("/activities/{id:guid}", async (Guid id, HttpRequest httpRequest, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(id, httpRequest.Headers.IfMatch.ToString(), db.Activities, cancellationToken);
    await service.DeleteActivityAsync(id, cancellationToken, expected);
    return Results.NoContent();
});
integrationProjects.MapGet("/activities/{id:guid}/comments", async (Guid id, HttpRequest request, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
...
integrationProjects.MapDelete("/activities/{activityId:guid}/comments/{commentId:guid}", async (Guid activityId, Guid commentId, HttpRequest httpRequest, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
{
    var expected = await rest.ReadExpectedVersionAsync(commentId, httpRequest.Headers.IfMatch.ToString(), db.ActivityComments, cancellationToken);
    await service.DeleteActivityCommentAsync(activityId, commentId, cancellationToken, expected);
    return Results.NoContent();
});
integrationProjects.MapGet("/activities/{id:guid}/pending-items", async (Guid id, HttpRequest request, ProjectService service, IntegrationRestSupport rest, OrganizaClubDbContext db, CancellationToken cancellationToken) =>
