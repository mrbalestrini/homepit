...
{
    private const string SpaceCreationScope = "spaces";
    private const string CoreCreationScope = "cores";
    ...
    public async Task EnsurePlanCatalogAsync(CancellationToken cancellationToken)
    {
        ...
        var missingPlans = PlanDefinitionCatalog.Defaults
            ...
            .Select(seed => new PlanDefinition
            {
                ...
                MaxOwnedSpaces = seed.MaxOwnedSpaces,
                MaxCores = seed.MaxCores,
        ...
        CancellationToken cancellationToken)
    {
        ...
        plan.MaxOwnedSpaces = request.MaxOwnedSpaces;
        plan.MaxCores = request.MaxCores;
    ...
    public async Task<CurrentUserPlanSummaryDto> GetCurrentUserPlanAsync(CancellationToken cancellationToken)
    {
        ...
        var ownedSpaceCount = await CountOwnedSpacesAsync(userContext.UserId, cancellationToken);
        var coreCount = await CountCreatedCoresAsync(userContext.UserId, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var activeMemberships = (await db.SpaceMembers
                .AsNoTracking()
                .Where(member => member.UserId == userContext.UserId && member.IsActive)
                .Select(member => new ActiveMembershipSnapshot(member.SpaceId, member.Id, member.Role))
                .ToArrayAsync(cancellationToken))
            .ToDictionary(member => member.SpaceId);

        return normalizedScope switch
        {
            SpaceCreationScope => await ListCreatedSpacesAsync(activeMemberships, cancellationToken),
            CoreCreationScope => await ListCreatedCoresAsync(activeMemberships, cancellationToken),
    ...
    public async Task EnsureCanCreateSpaceAsync(Guid userId, CancellationToken cancellationToken)
    {
        var plan = await ResolveEffectivePlanDefinitionAsync(userId, cancellationToken);
        var ownedSpaceCount = await CountOwnedSpacesAsync(userId, cancellationToken);

        if (ownedSpaceCount >= plan.MaxOwnedSpaces)
        {
            throw new ValidationException(BuildOwnedSpaceLimitMessage(plan));
        }
    }

    public async Task EnsureCanCreateCoreAsync(Guid userId, Guid spaceId, CancellationToken cancellationToken)
    {
        var plan = await ResolveEffectivePlanDefinitionAsync(userId, cancellationToken);
        var coreCount = await CountCreatedCoresAsync(userId, cancellationToken);

        if (coreCount >= plan.MaxCores)
        {
            throw new ValidationException(
                $"O plano {plan.Name} permite até {plan.MaxCores} núcleo(s) no total.");
        }
    }

    public async Task EnsureCanCreateProjectAsync(Guid userId, Guid coreId, CancellationToken cancellationToken)
    {
        var plan = await ResolveEffectivePlanDefinitionAsync(userId, cancellationToken);
    ...
    public async Task EnsureCanInviteMemberToSpaceAsync(
        Guid spaceId,
        Guid invitedUserId,
        CancellationToken cancellationToken)
    {
        var creatorUserId = await db.Spaces
            .AsNoTracking()
            .Where(space => space.Id == spaceId)
            .Select(space => space.CreatedByUserId)
    ...
    private static string BuildOwnedSpaceLimitMessage(PlanDefinition plan)
    {
        return plan.MaxOwnedSpaces == 0
            ? $"O plano {plan.Name} não permite criar espaços próprios."
            : $"O plano {plan.Name} permite até {plan.MaxOwnedSpaces} espaço(s) própria(s).";
    }

    private static string BuildInvitedMemberLimitMessage(PlanDefinition plan)
    {
        return plan.MaxInvitedMembers == 0
            ? $"O plano {plan.Name} não permite convidar membros para espaços próprios."
            : $"O plano {plan.Name} permite até {plan.MaxInvitedMembers} membro(s) convidado(s) ativo(s) nos espaços próprios.";
    }

    public async Task<PlanDefinition> ResolveEffectivePlanDefinitionForSpaceAsync(Guid spaceId, CancellationToken cancellationToken)
    {
        var ownerUserId = await db.Spaces
            .AsNoTracking()
            .Where(space => space.Id == spaceId)
            .Select(space => space.CreatedByUserId)
            .FirstOrDefaultAsync(cancellationToken);
    ...
    public async Task<PlanDefinition> ResolveEffectivePlanDefinitionForCoreAsync(Guid coreId, CancellationToken cancellationToken)
    {
        var spaceId = await db.Cores
            .AsNoTracking()
            .Where(core => core.Id == coreId)
            .Select(core => core.SpaceId)
            .FirstOrDefaultAsync(cancellationToken);
        ...
        return await ResolveEffectivePlanDefinitionForSpaceAsync(spaceId, cancellationToken);
    }

    public Task<int> CountOwnedSpacesAsync(Guid userId, CancellationToken cancellationToken)
    {
        return db.Spaces.CountAsync(space => space.CreatedByUserId == userId, cancellationToken);
    }

    public Task<int> CountCreatedCoresAsync(Guid userId, CancellationToken cancellationToken)
    {
        return db.Cores.CountAsync(
            core => core.CreatedByMember != null && core.CreatedByMember.UserId == userId,
            cancellationToken);
    }
    ...
    {
        return db.SpaceMembers.CountAsync(
            member =>
                member.IsActive &&
                member.Space != null &&
                member.Space.CreatedByUserId == creatorUserId &&
    ...
    private async Task<IReadOnlyCollection<PlanCreationItemDto>> ListCreatedSpacesAsync(
        IReadOnlyDictionary<Guid, ActiveMembershipSnapshot> activeMemberships,
        CancellationToken cancellationToken)
    {
        var spaces = await db.Spaces
            .AsNoTracking()
            .Where(space => space.CreatedByUserId == userContext.UserId)
        ...
        return spaces
            .Select(space => new PlanCreationItemDto(
                space.Id,
                space.Name,
                space.CreatedAt,
                space.Id,
                space.Name,
                activeMemberships.TryGetValue(space.Id, out var membership) && membership.Role == SpaceRole.Owner,
                null,
                null))
            .ToArray();
    }

    private async Task<IReadOnlyCollection<PlanCreationItemDto>> ListCreatedCoresAsync(
        IReadOnlyDictionary<Guid, ActiveMembershipSnapshot> activeMemberships,
        CancellationToken cancellationToken)
    {
        var cores = await db.Cores
            .AsNoTracking()
            .Where(core => core.CreatedByMember != null && core.CreatedByMember.UserId == userContext.UserId)
            ...
            .Select(core => new
            {
                ...
                core.CreatedAt,
                core.SpaceId,
                SpaceName = core.Space!.Name,
                core.CreatedByMemberId
            })
            .ToArrayAsync(cancellationToken);

        return cores
            .Select(core => new PlanCreationItemDto(
                core.Id,
                core.Name,
                core.CreatedAt,
                core.SpaceId,
                core.SpaceName,
                CanManageCreation(core.SpaceId, core.CreatedByMemberId, activeMemberships),
                null,
                null))
            .ToArray();
    }
        ...
        CancellationToken cancellationToken)
    {
        var projects = await db.Projects
            ...
            .Select(project => new
            {
                ...
                project.CreatedAt,
                project.SpaceId,
                SpaceName = project.Core!.Space!.Name,
                project.CoreId,
        ...
        return projects
            .Select(project => new PlanCreationItemDto(
                ...
                project.SpaceId,
                project.SpaceName,
                CanManageCreation(project.SpaceId, project.CreatedByMemberId, activeMemberships),
                project.CoreId,
                project.CoreName))
            .ToArray();
    }
    ...
    private static bool CanManageCreation(
        ...
        IReadOnlyDictionary<Guid, ActiveMembershipSnapshot> activeMemberships)
    {
        if (!activeMemberships.TryGetValue(spaceId, out var membership))
        {
            return false;
        }

        return membership.Role is SpaceRole.Owner or SpaceRole.Admin || membership.Id == createdByMemberId;
    }
    ...
    private static string NormalizeCreationScope(string value)
    {
        ...
        {
            SpaceCreationScope => SpaceCreationScope,
            CoreCreationScope => CoreCreationScope,
