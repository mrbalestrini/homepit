    ...
    IOptions<IntegrationOptions> options)
{
    ...
    public async Task<IReadOnlyCollection<IntegrationConnectionDto>> ListCurrentUserAsync(CancellationToken cancellationToken)
    {
        ...
        return await db.IntegrationConnections
            .AsNoTracking()
            .Include(connection => connection.Space)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var secret = Convert.ToBase64String(RandomNumberGenerator.GetBytes(32))
            ...
            .Replace('+', '-')
            .Replace('/', '_');
        var token = $"orgc_{keyId}_{secret}";
        ...
        var dto = ToDto(connection, membership.Space!.Name, now);
    ...
    {
        if (!userContext.IsIntegration || userContext.SpaceId is not Guid spaceId)
        {
    ...
    public async Task<IntegrationAuthenticatedPrincipal?> AuthenticateAsync(string token, CancellationToken cancellationToken)
    {
        ...
        var membership = await db.SpaceMembers
            ...
            .FirstOrDefaultAsync(member =>
                ...
                member.SpaceId == connection.SpaceId &&
                member.IsActive,
                cancellationToken);
    ...
    private static IntegrationConnectionDto ToDto(IntegrationConnection connection, string spaceName, DateTimeOffset now)
        => new(
            connection.Id,
            connection.Name,
            connection.CredentialKind,
