...
public sealed record IntegrationUniverseDto(Guid Id, string Name, bool HasImage, DateTimeOffset? ImageUpdatedAt, Guid? CreatedByMemberId, int ProjectCount, bool IsOutOfPlan);
public sealed record IntegrationProjectDto(Guid Id, Guid UniverseId, string UniverseName, bool UniverseHasImage, DateTimeOffset? UniverseImageUpdatedAt, string Name, Guid? CreatedByMemberId, int ActivityCount, bool IsOutOfPlan);
public sealed record IntegrationActivityDto(Guid Id, Guid ProjectId, string ProjectName, Guid UniverseId, string UniverseName, bool UniverseHasImage, DateTimeOffset? UniverseImageUpdatedAt, Guid? CreatedByMemberId, DateTimeOffset CreatedAt, string Title, string? Description, bool HasImage, DateTimeOffset? ImageUpdatedAt, DateOnly? DueDate, DateTimeOffset? CompletedAt, object Status, object Priority, decimal? Size, Guid? ResponsibleMemberId, string? ResponsibleName, int PendingCount, int CommentCount);

public static class IntegrationExternalDto
{
    public static IntegrationUniverseDto ToExternal(UniverseDto item) => new(item.Id, item.Name, item.HasImage, item.ImageUpdatedAt, item.CreatedByMemberId, item.ProjectCount, item.IsOutOfPlan);
    public static IntegrationProjectDto ToExternal(ProjectDto item) => new(item.Id, item.UniverseId, item.UniverseName, item.UniverseHasImage, item.UniverseImageUpdatedAt, item.Name, item.CreatedByMemberId, item.ActivityCount, item.IsOutOfPlan);
    public static IntegrationActivityDto ToExternal(ActivityDto item) => new(item.Id, item.ProjectId, item.ProjectName, item.UniverseId, item.UniverseName, item.UniverseHasImage, item.UniverseImageUpdatedAt, item.CreatedByMemberId, item.CreatedAt, item.Title, item.Description, item.HasImage, item.ImageUpdatedAt, item.DueDate, item.CompletedAt, item.Status, item.Priority, item.Size, item.ResponsibleMemberId, item.ResponsibleName, item.PendingCount, item.CommentCount);
}
