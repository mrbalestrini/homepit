    ...
    IOptions<OAuthOptions> options)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.AccessMode == IntegrationAccessMode.ReadWrite && !requestedScopes.Contains("homepit.write"))
        {
        ...
        var membership = await db.HouseholdMembers
            ...
            .FirstOrDefaultAsync(item => item.UserId == userContext.UserId && item.HouseholdId == request.HouseholdId && item.IsActive, cancellationToken)
            ?? throw new ForbiddenException("Você não tem acesso a esta casa.");

        var grantedScopes = requestedScopes
            .Where(scope => scope != "homepit.write" || request.AccessMode == IntegrationAccessMode.ReadWrite)
            .ToArray();
    ...
    private void ValidateAuthorizationRequest(OpenIddictRequest request)
    {
        ...
        {
            throw new ValidationException("A solicitação OAuth é inválida ou não está vinculada ao MCP HomePit.");
        }

        var scopes = request.GetScopes().ToArray();
        if (!scopes.Contains("homepit.read") || scopes.Any(scope => !AllowedScopes.Contains(scope)))
        {
