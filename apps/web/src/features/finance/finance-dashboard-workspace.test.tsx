...
function createDashboard(overrides: Partial<FinanceDashboardController> = {}): FinanceDashboardController {
  ...
  return {
    session: {
      ...
      spaces: [{ id: "house-1", name: "Espaço Financeira", role: "Owner" }],
    },
    activeSpaceId: "house-1",
    activeSpace: { id: "house-1", name: "Espaço Financeira", role: "Owner" },
    members: [],
    cores: [{ id: "core-1", name: "Espaço", imageUrl: null, hasImage: false, imageUpdatedAt: null, createdByMemberId: "member-1", projectCount: 1, canEdit: true, canDelete: true }],
    projects: [{ id: "project-1", coreId: "core-1", coreName: "Espaço", coreImageUrl: null, coreHasImage: false, coreImageUpdatedAt: null, name: "Moradia", createdByMemberId: "member-1", activityCount: 0, canEdit: true, canDelete: true }, { id: "project-2", coreId: "core-1", coreName: "Espaço", coreImageUrl: null, coreHasImage: false, coreImageUpdatedAt: null, name: "Viagem", createdByMemberId: "member-1", activityCount: 0, canEdit: true, canDelete: true }],
    categories,
...
describe("FinanceDashboardWorkspace", () => {
  ...
  it("opens the categories modal from the toolbar and allows starting a new category", async () => {
    ...
    expect(screen.getAllByText("Espaço").length).toBeGreaterThan(0);
  ...
  it("edits a custom category inline and keeps default categories read-only", async () => {
    ...
    const defaultRow = within(dialog)
      .getAllByRole("row")
      .find((row) => row.textContent?.includes("Espaço"));
    expect(defaultRow).not.toBeNull();
    expect(within(defaultRow!).queryByRole("button", { name: "Editar nome da categorio Espaço" })).toBeNull();
  });
