...
export function ProjectDashboardAuth({
  onAuthenticated,
}: {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(exception instanceof Error ? exception.message : "Falha ao autenticar.");
    } finally {
      setLoading(false);
    }
  }
        ...
        <section className="relative overflow-hidden rounded-[30px] border border-border/70 bg-surface p-8 shadow-lg backdrop-blur-md sm:p-10">
          ...
          <div className="relative space-y-8">
            <div className="flex items-center gap-3">
              <div className="grid size-14 place-items-center rounded-[20px] bg-primary text-primary-foreground shadow-sm">
                <Home className="size-6" />
              </div>
              <div>
                <p className="font-display text-3xl leading-none text-foreground">HomePit</p>
                <p className="mt-1 text-sm text-muted-foreground">Projetos da casa com estrutura e contexto.</p>
              </div>
            </div>

            <div className="space-y-5">
              <Badge className="rounded-full px-3 py-1 text-[12px]" variant="default">
                Sistema residencial premium
              </Badge>
              <div className="space-y-3">
                <h1 className="font-display text-4xl leading-tight text-foreground sm:text-5xl">
                  Organize obras, melhorias e rotinas da casa sem virar um SaaS genérico.
                </h1>
                <p className="max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg">
                  Estruture tudo em casas, universos, projetos e atividades, com visão operacional clara e espaço para
                  crescer para mercado, finanças e automações.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <HighlightCard
                ...
                title="Escopo claro"
                description="Universos e projetos bem organizados desde o primeiro acesso."
              />
              <HighlightCard
              ...
              <HighlightCard
                ...
                title="Operação elegante"
                description="Acompanhe o trabalho aberto com menos ruído visual."
              />
            </div>
          </div>
        </section>
          ...
          <CardContent className="p-6 sm:p-8">
            <form className="space-y-6" onSubmit={submit}>
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {mode === "login"
                      ? "Retome seus projetos e atividades de onde parou."
                      : "Cadastre-se e opcionalmente já crie a primeira casa do sistema."}
                  </p>
                </div>
              </div>
              ...
              <Button className="w-full" size="lg" type="submit" disabled={loading}>
                {loading ? "Aguarde..." : mode === "login" ? "Entrar no HomePit" : "Criar conta"}
              </Button>
            </form>
          </CardContent>
        </Card>
